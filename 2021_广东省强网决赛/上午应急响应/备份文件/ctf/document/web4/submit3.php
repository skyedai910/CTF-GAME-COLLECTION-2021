...
$setmoney=round($row['money']*$rate/100,2);
$DB->query("update `pay_order` set `type` ='{$type}',`addtime` ='{$date}' ,`addmoney` ='".$setmoney."',`rate`='{$rate}' where `trade_no`='{$trade_no}'");
if ($type == 'alipay') {
    if ($userrow['alipay']==2) {
       sysmsg("商户支付宝支付功能被关闭，无法正常支付，请选择其他支付方式！");
    }
     if($conf['alipay_api'] == 1) {
        echo "<script>window.location.href='./submit/alipay/alipay.php?trade_no={$trade_no}&type={$type}&name=充值余额&money={$row['money']}&sitename={$sitename}';</script>";
    } elseif($conf['alipay_api'] == 0) {
        exit($conf['ali_close_info']);
    } elseif ($conf['alipay_api'] == 3) {
        echo "<script>window.location.href='./submit/codepay/msubmit.php?trade_no={$trade_no}&type={$type}&name=充值余额&money={$row['money']}&sitename={$sitename}';</script>";
    } elseif ($conf['alipay_api'] == 2) {
        echo "<script>window.location.href='./submit/epay/epay.php?trade_no={$trade_no}&type={$type}&name=充值余额&money={$row['money']}&sitename={$sitename}';</script>";
 ...
 if ($conf['wxpay_api'] == 0) {
    ...
    echo "<script>window.location.href='./submit/alipay.php?trade_no={$trade_no}&sitename={$sitename}';</script>";
}
?>
<p>正在为您跳转到支付页面，请稍候...</p>
</body>
